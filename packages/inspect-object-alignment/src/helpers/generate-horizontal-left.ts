...
export const generateHorizontalLeft = (
  left: number,
  asGroup: AsGroup = false,
): fnFunction => ({ object, objects }) => ({
  left:
    object.left +
    (left - getBoundingRect(object).boxLeft) +
    (asGroup ? getBoundingRect(object).boxLeft - getGroupBox(objects).left : 0),
});
